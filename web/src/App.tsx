...
export function App() {
  ...
  useEffect(() => {
    ...
    return onSnapshot(q, snap =>
      setTasks(snap.docs.map(d => ({ id: d.id, data: d.data() as Task })))
    );
  }, []);

  async function submitNL() {
    if (!text.trim()) return;
    await createTaskFromNL(text.trim()); // no uid
    setText("");
  }
  ...
  async function submitManual() {
    ...
    await addDoc(col, {
      ...
      Participants: manual.participants.split(",").map(s => s.trim()).filter(Boolean),
        ...
        <section className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-1 lg:grid-cols-2">
            {tasks.map(({ id, data }) => (
              <article
                key={id}
                className="group bg-white/70 backdrop-blur-sm rounded-2xl shadow-lg hover:shadow-xl border border-white/50 p-6 transition-all duration-200 hover:scale-[1.02]"
              >
